...
contract LendingCore is ReentrancyGuard {
    ...
    function submitBid(
        ...
        bids[bidId] = Bid({
            ...
            recommendedAprBps: recommendedApr,
            open: true
        });
        emit BidSubmitted(
            bidId,
    ...
    function repay(uint256 loanId, uint256 amount) external nonReentrant {
        ...
        // Enforce interest-only minimum (capped at outstanding)
        uint256 minPay = minimumPayment(loanId);
        // Allow exact close (amount >= dueNow) OR paying at least the min required
        require(amount >= minPay, "below min");

        uint256 pay = amount > dueNow ? dueNow : amount;

        USDC.safeTransferFrom(msg.sender, L.lender, pay);
    ...
    function outstanding(uint256 loanId) public view returns (uint256) {
        ...
        uint256 baseInterest = InterestMath.calcBaseInterest(
            L.principal,
            L.aprBps,
            elapsedToDue
        );
        ...
        if (t > L.due) {
            ...
            if (capped > L.due) {
                ...
                penalty = InterestMath.calcPenaltyPerDays(
                    L.principal,
                    PENALTY_BPS_PER_DAY,
                    daysLate
                );
    ...
    function _interestOnly(Loan storage L) internal view returns (uint256) {
        uint256 t = block.timestamp;
    ...
    function minimumPayment(uint256 loanId) public view returns (uint256) {
        Loan storage L = loans[loanId];
        if (L.borrower == address(0)) return 0;

        uint256 interestOnly = _interestOnly(L);

        // Prior repayments are applied interest-first logically: the portion that could have gone to interest
        uint256 interestCovered = L.repaid > interestOnly
            ? interestOnly
            : L.repaid;
        uint256 stillDueInterest = interestOnly - interestCovered;

        uint256 dueNow = outstanding(loanId);
        // You can always close the loan by paying full outstanding; min is capped at outstanding
        return stillDueInterest > dueNow ? dueNow : stillDueInterest;
    }
}
